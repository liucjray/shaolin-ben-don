...
func PerformLoginIfRequired(ctx context.Context, client client.Client, config *config.Config, err error) (*LoggedInData, error) {
...
func (action *LoginAction) request(data *parser.LoginForm) (*http.Request, error) {
	...
	f := &form.LoginForm{
		Username: action.Config.App.Account,
		Password: action.Config.App.Password,
		Answer:   answer,
	}
